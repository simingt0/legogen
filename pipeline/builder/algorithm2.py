"""
Algorithm 2: Exhaustive Piece-by-Piece with Custom Priority Order
Tries to place each brick type exhaustively using a custom priority ranking.
"""
...
class Algorithm2(BuilderAlgorithm):
    """
    Exhaustive piece-by-piece algorithm with custom priority ordering.

    Strategy:
    - Process each layer from bottom to top
    - For each brick type in custom priority order:
    ...
    @property
    def name(self) -> str:
        return "Algorithm 2: Custom Priority Order"
    ...
    def build(
        ...
        """Generate LEGO build instructions using custom priority order."""
        ...
        for z in range(height):
            ...
            # Try each brick type in custom priority order
