"""
Algorithm 3: Floating Cell Priority with Simple Orphan Prevention
Prioritizes filling floating cells first, with simple rules to avoid orphaning.
"""
...
class Algorithm3(BuilderAlgorithm):
    """
    Floating cell priority algorithm with simple orphan prevention.
    ...
    - Only place bricks on floating cells if:
        ...
        * It's the last floating cell
    - For non-floating cells, use random placement
    """

    @property
    def name(self) -> str:
        return "Algorithm 3: Floating Cell Priority"

    @property
    def description(self) -> str:
        return (
            "Identifies floating cells (no vertical neighbor) and fills them first. "
            "Uses simple rule: only place on floating cells if covering multiple or it's the last one."
        )
    ...
    def build(
        ...
        for z, _ in sorted_layers:
            ...
            while remaining_floating:
                ...
                # Find best placements for floating cells
                best_placements = []
                max_floating_covered = 0
                    ...
                    for x in range(width):
                        for y in range(depth):
                            for rotation in [0, 90]:
                                ...
                                # Simple orphan prevention rule
                                if covered_floating == 1 and num_floating > 1:
                                    continue  # Don't place if only covers 1 and others remain

                                # Track best placements
                                if covered_floating > max_floating_covered:
                                    max_floating_covered = covered_floating
                                    best_placements = [(x, y, brick_type, rotation)]
                                elif covered_floating == max_floating_covered:
                                    best_placements.append((x, y, brick_type, rotation))
                ...
                # Randomly select among best placements
                x, y, brick_type, rotation = random.choice(best_placements)
